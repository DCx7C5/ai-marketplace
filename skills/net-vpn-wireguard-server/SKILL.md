---
name: net-vpn-wireguard-server
description: Deploy a WireGuard VPN server with peer management, split-tunnel routing, DNS leak prevention, and firewall rules for road-warrior and site-to-site configurations. domain: cybersecurity subdomain: network-security tags: - wireguard - vpn - peer - split-tunnel - dns-leak - kernel-module nist_csf: - PR.DS-02 - PR.IR-01 capec: []
domain: cybersecurity
---
## Overview

Deploy a WireGuard VPN server with peer management, split-tunnel routing, DNS leak prevention, and firewall rules for road-warrior and site-to-site configurations.
domain: cybersecurity
subdomain: network-security
tags:
  - wireguard
  - vpn
  - peer
  - split-tunnel
  - dns-leak
  - kernel-module
nist_csf:
  - PR.DS-02
  - PR.IR-01
capec: []

## Reference

See [SKILL taxonomy](../../TAXONOMY.md) for more details.